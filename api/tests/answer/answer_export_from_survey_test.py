...
@pytest.mark.asyncio
async def test_export_answers_csv_success(db_session, researcher_token):
    # Arrange
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get(
            f"/surveys/{survey.id}/answers/export/csv?filename=test_export",
    ...
    assert response.headers["content-disposition"] == "attachment; filename=test_export.csv"
    assert "user,Test Question" in response.text

@pytest.mark.asyncio
async def test_export_answers_excel_success(db_session, researcher_token):
    # Arrange
    category = Category(name="Test Category")
    db_session.add(category)
    await db_session.flush()
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get(
            f"/surveys/{survey.id}/answers/export/excel?filename=test_export",
            headers={"Authorization": f"Bearer {researcher_token}"}
        )
    ...
    assert response.headers["content-disposition"] == "attachment; filename=test_export.xlsx"

@pytest.mark.asyncio
async def test_export_answers_unauthorized(db_session):
    # Arrange
    survey_id = str(uuid.uuid4())

    # Act
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get(f"/surveys/{survey_id}/answers/export/csv?filename=test_export")

    # Assert
    assert response.status_code == 401

@pytest.mark.asyncio
async def test_export_answers_survey_not_found(db_session, researcher_token):
    # Arrange
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get(
            f"/surveys/{nonexistent_survey_id}/answers/export/csv?filename=test_export",
            headers={"Authorization": f"Bearer {researcher_token}"}
        )
...
@pytest.mark.asyncio
async def test_export_answers_not_assigned(db_session, participant_token):
    # Arrange
    category = Category(name="Test Category")
    db_session.add(category)
    await db_session.flush()
...
@pytest.mark.asyncio
async def test_export_answers_not_in_organization(db_session, researcher_token):
    # Arrange
    category = Category(name="Test Category")
    db_session.add(category)
    await db_session.flush()
